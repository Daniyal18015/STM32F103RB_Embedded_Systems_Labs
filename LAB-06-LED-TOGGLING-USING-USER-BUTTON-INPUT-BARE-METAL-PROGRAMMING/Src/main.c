...
int main(void)
{
	...
	while(1)
	{
	  // When the BUTTON at (PC13) get pressed
		if((*(volatile unsigned int *)(GPIO_BASE_ADDRESS + (GPIO_PORT_OFFSET*GPIO_PORT_C) + GPIO_IDR_OFFSET)) & (0x1 << (GPIO_PIN_13)))
		{
		  // LED at (PA5) Turn ON
			(*(volatile unsigned int *)(GPIO_BASE_ADDRESS + (GPIO_PORT_OFFSET*GPIO_PORT_A) + GPIO_BSRR_OFFSET) = (0x1 << ((GPIO_PIN_5*4)+1)));
		}
		else
		{
		  // LED at (PA5) Turn OFF
			(*(volatile unsigned int *)(GPIO_BASE_ADDRESS + (GPIO_PORT_OFFSET*GPIO_PORT_A) + GPIO_BSRR_OFFSET) = (0x1 << (GPIO_PIN_5)));
		}
	}
}
